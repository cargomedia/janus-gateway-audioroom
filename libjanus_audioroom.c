...
int cm_audiobridge_init(janus_callbacks *callback, const char *config_path);
void cm_audiobridge_destroy(void);
int cm_audiobridge_get_api_compatibility(void);
int cm_audiobridge_get_version(void);
const char *cm_audiobridge_get_version_string(void);
const char *cm_audiobridge_get_description(void);
const char *cm_audiobridge_get_name(void);
const char *cm_audiobridge_get_author(void);
const char *cm_audiobridge_get_package(void);
void cm_audiobridge_create_session(janus_plugin_session *handle, int *error);
struct janus_plugin_result *cm_audiobridge_handle_message(janus_plugin_session *handle, char *transaction, char *message, char *sdp_type, char *sdp);
void cm_audiobridge_setup_media(janus_plugin_session *handle);
void cm_audiobridge_incoming_rtp(janus_plugin_session *handle, int video, char *buf, int len);
void cm_audiobridge_incoming_rtcp(janus_plugin_session *handle, int video, char *buf, int len);
void cm_audiobridge_hangup_media(janus_plugin_session *handle);
void cm_audiobridge_destroy_session(janus_plugin_session *handle, int *error);
char *cm_audiobridge_query_session(janus_plugin_session *handle);

/* Plugin setup */
static janus_plugin cm_audiobridge_plugin =
	JANUS_PLUGIN_INIT (
		.init = cm_audiobridge_init,
		.destroy = cm_audiobridge_destroy,

		.get_api_compatibility = cm_audiobridge_get_api_compatibility,
		.get_version = cm_audiobridge_get_version,
		.get_version_string = cm_audiobridge_get_version_string,
		.get_description = cm_audiobridge_get_description,
		.get_name = cm_audiobridge_get_name,
		.get_author = cm_audiobridge_get_author,
		.get_package = cm_audiobridge_get_package,

		.create_session = cm_audiobridge_create_session,
		.handle_message = cm_audiobridge_handle_message,
		.setup_media = cm_audiobridge_setup_media,
		.incoming_rtp = cm_audiobridge_incoming_rtp,
		.incoming_rtcp = cm_audiobridge_incoming_rtcp,
		.hangup_media = cm_audiobridge_hangup_media,
		.destroy_session = cm_audiobridge_destroy_session,
		.query_session = cm_audiobridge_query_session,
	);

/* Plugin creator */
janus_plugin *create(void) {
	JANUS_LOG(LOG_VERB, "%s created!\n", CM_AUDIOBRIDGE_NAME);
	return &cm_audiobridge_plugin;
}
...
static void *cm_audiobridge_handler(void *data);
static void cm_audiobridge_relay_rtp_packet(gpointer data, gpointer user_data);
static void *cm_audiobridge_mixer_thread(void *data);
static void *cm_audiobridge_participant_thread(void *data);

typedef struct cm_audiobridge_message {
	janus_plugin_session *handle;
	char *transaction;
	json_t *message;
	char *sdp_type;
	char *sdp;
} cm_audiobridge_message;
static GAsyncQueue *messages = NULL;

void cm_audiobridge_message_free(cm_audiobridge_message *msg);
void cm_audiobridge_message_free(cm_audiobridge_message *msg) {
	if(!msg)
		return;
...
typedef struct cm_audiobridge_room {
...
typedef struct cm_audiobridge_session {
	janus_plugin_session *handle;
...
} cm_audiobridge_session;
static GHashTable *sessions;
static GList *old_sessions;
static janus_mutex sessions_mutex;

typedef struct cm_audiobridge_rtp_context {
	/* Needed to fix seq and ts in case of publisher switching */
	uint32_t a_last_ssrc, a_last_ts, a_base_ts, a_base_ts_prev;
	uint16_t a_last_seq, a_base_seq, a_base_seq_prev;
} cm_audiobridge_rtp_context;

typedef struct cm_audiobridge_participant {
	cm_audiobridge_session *session;
	cm_audiobridge_room *room;	/* Room */
	...
	cm_audiobridge_rtp_context context;	/* Needed in case the participant changes room */
	/* Opus stuff */
...
} cm_audiobridge_participant;

/* Packets we get from gstreamer and relay */
typedef struct cm_audiobridge_rtp_relay_packet {
	rtp_header *data;
	gint length;
	uint32_t ssrc;
	uint32_t timestamp;
	uint16_t seq_number;
} cm_audiobridge_rtp_relay_packet;

/* Helper to sort incoming RTP packets by sequence numbers */
static gint cm_audiobridge_rtp_sort(gconstpointer a, gconstpointer b) {
	cm_audiobridge_rtp_relay_packet *pkt1 = (cm_audiobridge_rtp_relay_packet *)a;
	cm_audiobridge_rtp_relay_packet *pkt2 = (cm_audiobridge_rtp_relay_packet *)b;
...
/* Error codes */
#define CM_AUDIOBRIDGE_ERROR_UNKNOWN_ERROR	499
#define CM_AUDIOBRIDGE_ERROR_NO_MESSAGE		480
#define CM_AUDIOBRIDGE_ERROR_INVALID_JSON	481
#define CM_AUDIOBRIDGE_ERROR_INVALID_REQUEST	482
#define CM_AUDIOBRIDGE_ERROR_MISSING_ELEMENT	483
#define CM_AUDIOBRIDGE_ERROR_INVALID_ELEMENT	484
#define CM_AUDIOBRIDGE_ERROR_NO_SUCH_ROOM	485
#define CM_AUDIOBRIDGE_ERROR_ROOM_EXISTS		486
#define CM_AUDIOBRIDGE_ERROR_NOT_JOINED		487
#define CM_AUDIOBRIDGE_ERROR_LIBOPUS_ERROR	488
#define CM_AUDIOBRIDGE_ERROR_UNAUTHORIZED	489
#define CM_AUDIOBRIDGE_ERROR_ID_EXISTS		490
#define CM_AUDIOBRIDGE_ERROR_ALREADY_JOINED	491


/* AudioBridge watchdog/garbage collector (sort of) */
void *cm_audiobridge_watchdog(void *data);
void *cm_audiobridge_watchdog(void *data) {
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(old_sessions != NULL) {
			...
			while(sl) {
				cm_audiobridge_session *session = (cm_audiobridge_session *)sl->data;
				if(!session) {
					sl = sl->next;
					continue;
				}
...
int cm_audiobridge_init(janus_callbacks *callback, const char *config_path) {
	...
	/* Read configuration */
	char filename[255];
	g_snprintf(filename, 255, "%s/%s.cfg", config_path, CM_AUDIOBRIDGE_PACKAGE);
	...
	messages = g_async_queue_new_full((GDestroyNotify) cm_audiobridge_message_free);
	...
	if(config != NULL) {
		...
		while(cat != NULL) {
			...
			cm_audiobridge_room *audiobridge = g_malloc0(sizeof(cm_audiobridge_room));
			if(audiobridge == NULL) {
			...
			GError *error = NULL;
			audiobridge->thread = g_thread_try_new("audiobridge mixer thread", &cm_audiobridge_mixer_thread, audiobridge, &error);
			if(error != NULL) {
	...
	watchdog = g_thread_try_new("abridge watchdog", &cm_audiobridge_watchdog, NULL, &error);
	if(error != NULL) {
	...
	handler_thread = g_thread_try_new("janus audiobridge handler", cm_audiobridge_handler, NULL, &error);
	if(error != NULL) {
		g_atomic_int_set(&initialized, 0);
...
int cm_audiobridge_get_version(void) {
	return CM_AUDIOBRIDGE_VERSION;
}

const char *cm_audiobridge_get_version_string(void) {
	return CM_AUDIOBRIDGE_VERSION_STRING;
}

const char *cm_audiobridge_get_description(void) {
	return CM_AUDIOBRIDGE_DESCRIPTION;
}

const char *cm_audiobridge_get_name(void) {
	return CM_AUDIOBRIDGE_NAME;
}

const char *cm_audiobridge_get_author(void) {
	return CM_AUDIOBRIDGE_AUTHOR;
}

const char *cm_audiobridge_get_package(void) {
	return CM_AUDIOBRIDGE_PACKAGE;
}
...
void cm_audiobridge_create_session(janus_plugin_session *handle, int *error) {
	...
	cm_audiobridge_session *session = (cm_audiobridge_session *)g_malloc0(sizeof(cm_audiobridge_session));
	if(session == NULL) {
		JANUS_LOG(LOG_FATAL, "Memory error!\n");
		*error = -2;
		return;
...
void cm_audiobridge_destroy_session(janus_plugin_session *handle, int *error) {
	...
	cm_audiobridge_session *session = (cm_audiobridge_session *)handle->plugin_handle;
	if(!session) {
	...
	if(!session->destroyed) {
		...
		cm_audiobridge_hangup_media(handle);
...
char *cm_audiobridge_query_session(janus_plugin_session *handle) {
	...
	cm_audiobridge_participant *participant = (cm_audiobridge_participant *)session->participant;
	json_object_set_new(info, "state", json_string(participant && participant->room ? "inroom" : "idle"));
	if(participant) {
		cm_audiobridge_room *room = participant->room;
...
struct janus_plugin_result *cm_audiobridge_handle_message(janus_plugin_session *handle, char *transaction, char *message, char *sdp_type, char *sdp) {
	...
	if(message == NULL) {
		...
		error_code = CM_AUDIOBRIDGE_ERROR_NO_MESSAGE;
	...
	if(!session) {
		JANUS_LOG(LOG_ERR, "No session associated with this handle...\n");
		error_code = CM_AUDIOBRIDGE_ERROR_UNKNOWN_ERROR;
	...
	if(!root) {
		...
		error_code = CM_AUDIOBRIDGE_ERROR_INVALID_JSON;
	...
	if(!request) {
		...
		error_code = CM_AUDIOBRIDGE_ERROR_MISSING_ELEMENT;
		g_snprintf(error_cause, 512, "Missing element (request)");
		goto error;
	}
	if(!json_is_string(request)) {
		JANUS_LOG(LOG_ERR, "Invalid element (request should be a string)\n");
		error_code = CM_AUDIOBRIDGE_ERROR_INVALID_ELEMENT;
	...
	if(!strcasecmp(request_text, "create")) {
		...
		if(room_id > 0) {
			...
			if(g_hash_table_lookup(rooms, GUINT_TO_POINTER(room_id)) != NULL) {
				...
				error_code = CM_AUDIOBRIDGE_ERROR_ROOM_EXISTS;
		...
		janus_mutex_lock(&rooms_mutex);
		cm_audiobridge_room *audiobridge = g_hash_table_lookup(rooms, GUINT_TO_POINTER(room_id));
		if(audiobridge == NULL) {
			janus_mutex_unlock(&rooms_mutex);
			JANUS_LOG(LOG_ERR, "No such room (%"SCNu64")\n", room_id);
			error_code = CM_AUDIOBRIDGE_ERROR_NO_SUCH_ROOM;
		...
		if(audiobridge->room_secret) {
			...
			if(!janus_strcmp_const_time(audiobridge->room_secret, json_string_value(secret))) {
				...
				error_code = CM_AUDIOBRIDGE_ERROR_UNAUTHORIZED;
		...
		while (g_hash_table_iter_next(&iter, NULL, &value)) {
			cm_audiobridge_participant *p = value;
			if(p && p->session) {
				p->room = NULL;
				int ret = gateway->push_event(p->session->handle, &cm_audiobridge_plugin, NULL, response_text, NULL, NULL);
				...
				while(p->inbuf) {
					...
					cm_audiobridge_rtp_relay_packet *pkt = (cm_audiobridge_rtp_relay_packet *)first->data;
		...
		g_hash_table_iter_init(&iter, rooms);
		while(g_hash_table_iter_next(&iter, NULL, &value)) {
			cm_audiobridge_room *room = value;
			if(!room)
				continue;
			if(room->is_private) {
				/* Skip private room */
		...
		if(participant == NULL || participant->room == NULL) {
			...
			error_code = CM_AUDIOBRIDGE_ERROR_NOT_JOINED;
		...
		cm_audiobridge_message *msg = g_malloc0(sizeof(cm_audiobridge_message));
		if(msg == NULL) {
			JANUS_LOG(LOG_FATAL, "Memory error!\n");
			error_code = CM_AUDIOBRIDGE_ERROR_UNKNOWN_ERROR;
			g_snprintf(error_cause, 512, "Memory error");
		...
		error_code = CM_AUDIOBRIDGE_ERROR_INVALID_REQUEST;
...
void cm_audiobridge_incoming_rtp(janus_plugin_session *handle, int video, char *buf, int len) {
	...
	if(participant->active && participant->decoder) {
		...
		cm_audiobridge_rtp_relay_packet *pkt = g_malloc0(sizeof(cm_audiobridge_rtp_relay_packet));
		if(pkt == NULL) {
			JANUS_LOG(LOG_FATAL, "Memory error!\n");
			return;
		}
		...
		participant->inbuf = g_list_insert_sorted(participant->inbuf, pkt, &cm_audiobridge_rtp_sort);
...
void cm_audiobridge_hangup_media(janus_plugin_session *handle) {
	...
	cm_audiobridge_participant *participant = (cm_audiobridge_participant *)session->participant;
	cm_audiobridge_room *audiobridge = participant->room;
	if(audiobridge != NULL) {
		janus_mutex_lock(&audiobridge->mutex);
		json_t *event = json_object();
		json_object_set_new(event, "audiobridge", json_string("event"));
		...
		while (g_hash_table_iter_next(&iter, NULL, &value)) {
			...
			int ret = gateway->push_event(p->session->handle, &cm_audiobridge_plugin, NULL, leaving_text, NULL, NULL);
			JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
		}
		g_free(leaving_text);
	}
...
static void *cm_audiobridge_handler(void *data) {
	...
	cm_audiobridge_message *msg = NULL;
	int error_code = 0;
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		cm_audiobridge_session *session = (cm_audiobridge_session *)msg->handle->plugin_handle;
		if(!session) {
			JANUS_LOG(LOG_ERR, "No session associated with this handle...\n");
			cm_audiobridge_message_free(msg);
			continue;
		}
		if(session->destroyed) {
			cm_audiobridge_message_free(msg);
		...
		if(!strcasecmp(request_text, "join")) {
			...
			cm_audiobridge_participant *participant = session->participant;
			if(participant != NULL && participant->room != NULL) {
				JANUS_LOG(LOG_ERR, "Already in a room (use changeroom to join another one)\n");
				error_code = CM_AUDIOBRIDGE_ERROR_ALREADY_JOINED;
			...
			if(id) {
				...
				if(g_hash_table_lookup(audiobridge->participants, GUINT_TO_POINTER(user_id)) != NULL) {
					...
					error_code = CM_AUDIOBRIDGE_ERROR_ID_EXISTS;
			...
			if(participant == NULL) {
				participant = g_malloc0(sizeof(cm_audiobridge_participant));
				if(participant == NULL) {
					JANUS_LOG(LOG_FATAL, "Memory error!\n");
					error_code = CM_AUDIOBRIDGE_ERROR_UNKNOWN_ERROR;
					g_snprintf(error_cause, 512, "Memory error");
			...
			if(participant->encoder == NULL) {
				...
				if(error != OPUS_OK) {
					...
					error_code = CM_AUDIOBRIDGE_ERROR_LIBOPUS_ERROR;
			...
			if(participant->thread == NULL) {
				GError *error = NULL;
				participant->thread = g_thread_try_new("audiobridge participant thread", &cm_audiobridge_participant_thread, participant, &error);
				if(error != NULL) {
			...
			while (g_hash_table_iter_next(&iter, NULL, &value)) {
				...
				JANUS_LOG(LOG_VERB, "Notifying participant %"SCNu64" (%s)\n", p->user_id, p->display ? p->display : "??");
				int ret = gateway->push_event(p->session->handle, &cm_audiobridge_plugin, NULL, newuser_text, NULL, NULL);
				JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
			}
			g_free(newuser_text);
			...
			if(muted) {
				...
				while (g_hash_table_iter_next(&iter, NULL, &value)) {
					...
					JANUS_LOG(LOG_VERB, "Notifying participant %"SCNu64" (%s)\n", p->user_id, p->display ? p->display : "??");
					int ret = gateway->push_event(p->session->handle, &cm_audiobridge_plugin, NULL, pub_text, NULL, NULL);
					JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
				}
				g_free(pub_text);
				janus_mutex_unlock(&audiobridge->mutex);
			...
			cm_audiobridge_room *old_audiobridge = participant->room;
		...
		/* Any SDP to handle? */
		if(!msg->sdp) {
			int ret = gateway->push_event(msg->handle, &cm_audiobridge_plugin, msg->transaction, event_text, NULL, NULL);
			JANUS_LOG(LOG_VERB, "  >> %d (%s)\n", ret, janus_get_api_error(ret));
		} else {
			...
			int res = gateway->push_event(msg->handle, &cm_audiobridge_plugin, msg->transaction, event_text, type, sdp);
...
static void *cm_audiobridge_mixer_thread(void *data) {
	...
	cm_audiobridge_room *audiobridge = (cm_audiobridge_room *)data;
	if(!audiobridge) {
	...
	while(!g_atomic_int_get(&stopping) && audiobridge->destroyed == 0) {	/* FIXME We need a per-room watchdog as well */
		...
		while(ps) {
			cm_audiobridge_participant *p = (cm_audiobridge_participant *)ps->data;
			janus_mutex_lock(&p->qmutex);
			...
			cm_audiobridge_rtp_relay_packet *pkt = (cm_audiobridge_rtp_relay_packet *)(peek ? peek->data : NULL);
			if(pkt != NULL) {
		...
		while(ps) {
			cm_audiobridge_participant *p = (cm_audiobridge_participant *)ps->data;
			cm_audiobridge_rtp_relay_packet *pkt = NULL;
			janus_mutex_lock(&p->qmutex);
			if(p->active && !p->muted && !p->prebuffering && p->inbuf) {
				GList *first = g_list_first(p->inbuf);
				pkt = (cm_audiobridge_rtp_relay_packet *)(first ? first->data : NULL);
			...
			cm_audiobridge_rtp_relay_packet *mixedpkt = g_malloc0(sizeof(cm_audiobridge_rtp_relay_packet));
			if(mixedpkt != NULL) {
...
static void *cm_audiobridge_participant_thread(void *data) {
	...
	cm_audiobridge_participant *participant = (cm_audiobridge_participant *)data;
	if(!participant) {
	...
	cm_audiobridge_session *session = participant->session;

	/* Output buffer */
	cm_audiobridge_rtp_relay_packet *outpkt = g_malloc0(sizeof(cm_audiobridge_rtp_relay_packet));
	if(outpkt == NULL) {
		JANUS_LOG(LOG_FATAL, "Memory error!\n");
		g_thread_unref(g_thread_self());
		return NULL;
	...
	cm_audiobridge_rtp_relay_packet *mixedpkt = NULL;
		...
		if(mixedpkt != NULL && session->destroyed == 0) {
			...
			if(participant->active && participant->encoder) {
				...
				if(outpkt->length < 0) {
					...
					cm_audiobridge_relay_rtp_packet(participant->session, outpkt);
				}
			}
			if(mixedpkt) {
				if(mixedpkt->data)
	...
	while(g_async_queue_length(participant->outbuf) > 0) {
		cm_audiobridge_rtp_relay_packet *pkt = g_async_queue_pop(participant->outbuf);
		if(pkt == NULL)
			continue;
		if(pkt->data)
			g_free(pkt->data);
...
static void cm_audiobridge_relay_rtp_packet(gpointer data, gpointer user_data) {
	cm_audiobridge_rtp_relay_packet *packet = (cm_audiobridge_rtp_relay_packet *)user_data;
	if(!packet || !packet->data || packet->length < 1) {
		JANUS_LOG(LOG_ERR, "Invalid packet...\n");
		return;
	}
	cm_audiobridge_session *session = (cm_audiobridge_session *)data;
